...
export class ClobWsApi {
  ...
  private handle(ws: WebSocket, message: WsClientMessage): void {
    ...
    switch (message.type) {
      ...
      case "submit_order": return this.submitOrder(ws, session, message.data);
      case "cancel_order": return this.cancelOrder(ws, session, message.data);
  ...
  private submitOrder(ws: WebSocket, session: Session, data: unknown): void {
    try {
  ...
  private cancelOrder(ws: WebSocket, session: Session, data: unknown): void {
    try {
      this.requireAuth(session);
