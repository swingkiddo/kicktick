...
pub struct CreateOrder<'info> {
    ...
    #[account(mut, seeds = [SEED_USER_VAULT, owner.key().as_ref()], bump = user_account.vault_bump)]
...
pub struct ExpireOrder<'info> {
    ...
    pub user_account: Account<'info, UserAccount>,
    #[account(mut, seeds = [SEED_USER_VAULT, order.owner.as_ref()], bump = user_account.vault_bump)]
    pub user_vault: SystemAccount<'info>,
...
fn release(order: &OrderAccount, user: &mut UserAccount, position: &mut Position) -> Result<()> {
    ...
    match order.side {
        OrderSide::Buy => {
            let amount = (remaining as u128 * order.price_bps as u128 / PRICE_SCALE_BPS as u128) as u64;
...
pub fn create_order_handler(ctx: Context<CreateOrder>, side: OrderSide, outcome_index: u8, price_bps: u16, quantity: u64, nonce: u64, expires_at: i64) -> Result<()> {
    ...
    require!(Clock::get()?.unix_timestamp < ctx.accounts.market.expires_at && expires_at <= ctx.accounts.market.expires_at, KickTickError::DeadlinePassed);
    ...
    match side {
        OrderSide::Buy => {
            let amount = ((quantity as u128 * price_bps as u128) / PRICE_SCALE_BPS as u128) as u64;
...
pub fn cancel_order_handler(ctx: Context<CancelOrder>) -> Result<()> {
    require!(ctx.accounts.order.status == OrderStatus::Open, KickTickError::OrderNotOpen);
